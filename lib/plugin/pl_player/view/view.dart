...
class _PLVideoPlayerState extends State<PLVideoPlayer>
  ...
  void _controlListener(bool val) {
    ...
    if (widget.videoDetailController case final controller?) {
      if (controller.vttSubtitlesIndex.value != 0 ||
          controller.secondarySubtitlesIndex.value != 0) {
        if (visible) {
  ...
  Widget _buildStyledSubtitleText(
    String text,
    TextStyle style,
  ...
  Widget _buildDualSubtitleView() {
    return Obx(() {
      plPlayerController.subtitleConfig.value;
      final primaryStyle = plPlayerController.getPrimarySubtitleTextStyle();
      final primaryStrokeStyle = plPlayerController
          .getPrimarySubtitleStrokeStyle();
      final secondaryStyle = plPlayerController.getSecondarySubtitleTextStyle();
      final secondaryStrokeStyle = plPlayerController
          .getSecondarySubtitleStrokeStyle();
      final secondaryOnTop = plPlayerController.subtitleTranslationOnTop;

      Widget child = StreamBuilder<dynamic>(
        stream: plPlayerController.videoPlayerController?.stream.subtitle,
        builder: (context, snapshot) {
          final dynamic subtitle = snapshot.data;
          final String primaryText = ('${subtitle?.primary ?? ''}').trim();
          final String secondaryText = ('${subtitle?.secondary ?? ''}').trim();
          ...
          return Align(
            ...
            child: Padding(
              padding: EdgeInsets.only(
                left: plPlayerController.subtitlePaddingH.toDouble(),
                right: plPlayerController.subtitlePaddingH.toDouble(),
                bottom: plPlayerController.subtitlePaddingB.toDouble(),
              ),
              child: Column(
      ...
      if (plPlayerController.enableDragSubtitle) {
        child = GestureDetector(
          behavior: HitTestBehavior.translucent,
          onVerticalDragUpdate: (details) {
            final newBottom =
                (plPlayerController.subtitlePaddingB - details.delta.dy)
                    .round()
                    .clamp(0, 200);
            plPlayerController
              ..subtitlePaddingB = newBottom
              ..updateSubtitleStyle();
          },
          onVerticalDragEnd: (_) => plPlayerController.putSubtitleSettings(),
          child: child,
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final child = Stack(
      ...
      children: <Widget>[
        ...
        if (!isLive) Positioned.fill(child: _buildDualSubtitleView()),
