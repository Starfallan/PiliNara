...
class VideoReplyReplyController extends ReplyController
  ...
  Future<void> onRefresh() {
    ...
    if (isSeedMode) {
      // 必须走重置链：isEnd / paginationReply / cursorNext 不重置的话，
      // 刷新只是"继续往下翻页"，且 isEnd 已为真时整个刷新会直接返回、
      // 下拉动画转完却什么都没发生。
      isEnd = false;
      index.value = null;
    }
  ...
  /// seed 模式增量续拉：从当前游标连续翻页，直到已覆盖已知子树规模、
  /// isEnd、或游标不再推进。
  ///
  /// 上界由**数据**导出而非固定魔数：seed 模式已知当前子树的总条数
  /// （`count` 即 `_refreshSeedCount()` 算出的子树长度）。拉到覆盖该规模
  /// 即可——继续翻只会把同楼层**其他子树**的回复也拉下来再被
  /// `extractSubtree` 丢掉。用户继续下滑会自然触发下一批。
  ///
  /// 判停用**游标是否推进**而非数组长度：分页用热度排序
  /// （`Mode.MAIN_LIST_HOT`），翻页期间点赞数变化会让边界条目跨页重复返回，
  /// 此时长度仍在增长，长度比较无法反映"没有新内容"。
  Future<void> _continueSeedLoad() async {
    if (isLoading || isEnd) return;
    // count 不可用时的兜底页数（按每页约 20 条估，覆盖 300 条足够厚）
    const fallbackSteps = 15;
    // 每页约 20 条（接口分页粒度）；用于把目标条数折算成页数
    const repliesPerPage = 20;
    final target = count.value;
    // 已知目标则只需拉到覆盖它的页数，留 2 页余量；未知则用兜底值
    final maxSteps = target > 0
        ? (target / repliesPerPage).ceil() + 2
        : fallbackSteps;
    var steps = 0;
    while (!isEnd && steps < maxSteps) {
      steps++;
      final beforeOffset = paginationReply?.nextOffset;
      final beforeLength = loadingState.value.data?.length ?? 0;
      await queryData(false);
      final afterOffset = paginationReply?.nextOffset;
      // 游标未推进 → 服务端不再给出新位置，再翻也是同一页
      if (afterOffset != null && afterOffset == beforeOffset) break;
      // 长度零增长且无游标可比较 → 同样视为无进展
      if (afterOffset == null &&
          (loadingState.value.data?.length ?? 0) <= beforeLength) {
        break;
      }
    }
  }
